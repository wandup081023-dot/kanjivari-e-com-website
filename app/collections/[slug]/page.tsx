...
const PRICE_BOUNDS = { min: 0, max: 6000 };
    ...
    <div className="space-y-3">
      <div className="relative h-6 flex items-center">
        ...
        <input type="range" min={min} max={max} step={100} value={value[0]}
          onChange={e => { const v = Math.min(Number(e.target.value), value[1] - 200); onChange([v, value[1]]); }}
          className="absolute w-full h-1 appearance-none bg-transparent cursor-pointer" style={{ zIndex: value[0] > max - 200 ? 5 : 3 }} />
        <input type="range" min={min} max={max} step={100} value={value[1]}
          onChange={e => { const v = Math.max(Number(e.target.value), value[0] + 200); onChange([value[0], v]); }}
